...
vector <pair<int, pll>> breakpoints; // <id, <addr, original_code>>
struct elf_s{
    FILE *fp;
...
void Launch(string execfile, string memory_content){
    ...
    if(ehdr.e_ident[EI_MAG0] == 0x7f && ehdr.e_ident[EI_MAG1] == 'E' && ehdr.e_ident[EI_MAG2] == 'L' && ehdr.e_ident[EI_MAG3] == 'F' && ehdr.e_ident[EI_CLASS] == ELFCLASS64){
        str_shdr_off = ehdr.e_shoff + ehdr.e_shstrndx * sizeof(Elf64_Shdr);
        fseek(fp, str_shdr_off, SEEK_SET);
        fread(&str_shdr, 1, sizeof(Elf64_Shdr), fp);
        fseek(fp, str_shdr.sh_offset, SEEK_SET);
        fread(str_tab, str_shdr.sh_size, sizeof(char), fp);
        fseek(fp, ehdr.e_shoff, SEEK_SET);
        for(int i = 0; i < ehdr.e_shnum; i++){
            fread(&str_shdr_, 1, sizeof(Elf64_Shdr), fp);
            if(strcmp((str_tab + str_shdr_.sh_name), ".data") == 0){
                text_shdr = str_shdr_;
                break;
            }
        }
        // printf("**program \'./%s\' loaded. data at 0x%lx\n", execfile.c_str(), ehdr.e_entry);
        pid_t pid = fork();
        if(pid < 0){
            perror("Fork Error");
            exit(0);
            ...
            while (WIFSTOPPED(state)) {
                ...
                if(counter == 3){
                    if(ptrace(PTRACE_GETREGS, pid, 0, &regs) == 0) {
                        rip = regs.rip;
                        for(int i = 0; i < 10; i++){
                            if(ptrace(PTRACE_POKETEXT, pid, regs.rax + i, memory_content[i]) != 0){
                ...
                if(waitpid(pid, &state, 0) < 0) errquit("waitpid");
            }
        }
    }
}
...
int main(int argc, char* argv[]){
    ...
    for(int i = 0; i < text.size() && !bingo; i++){
        Launch(execfile, text[i]);
    }
